...
let audiobar = document.getElementById("audio");

let PresetThemeSelected = document.getElementById("presetcolor");
let CustomThemeSelected = document.getElementById("custom");
let PresetContent = document.getElementById('typeColor');
let CustomContent = document.getElementById('yu');
...
function setTime() {
	...
	if (roundInfo.running) {
		document.title = `${timestr} ${fullname[roundInfo.current]}`;
	}else {
		document.title = 'FOCSTU';
	}
}
...
const accents = {
	dark: {
		red: {
			"--bgcolor": "oklch(from var(--coloraccent) calc(l*0.3) calc(c*0.2) h)",
			"--bgcolor2": "oklch(from var(--coloraccent) calc(l*0.4) calc(c*0.8) h)",
			"--bgcolor2": "#403333",
			"--color": "#ffeeee",
			"--coloraccent": "#ffaaaa",
		},
	...
	light: {
		red: {
			"--bgcolor": "oklch(from var(--coloraccent) calc(l*3) calc(c*0.3) h)",
			"--bgcolor2": "oklch(from var(--coloraccent) calc(l*1.7) calc(c*0.8) h)",
			"--color": "#222222",
			"--coloraccent": "#d64f4f",
		},
		orange: {
...
let themeType = "preset";
function themeTypeSelect() {
	if (themeType === "preset") {
		CustomContent.classList.add("disabled");
		PresetContent.classList.remove("disabled");
		localStorage.setItem("themetype", "preset");
		themeType = localStorage.getItem("themetype");
	} else {
		PresetContent.classList.add("disabled");
		CustomContent.classList.remove("disabled");
...
function checks() {
	if (themeType === "preset") {
	PresetThemeSelected.checked = true;
	CustomThemeSelected.checked = false;
	} else {
	PresetThemeSelected.checked = false;
	CustomThemeSelected.checked = true;
}
}

console.log(themeType);
console.log(localStorage.getItem("themetype"));

PresetThemeSelected.addEventListener("click", function() {
	themeType = "preset"
	themeTypeSelect();
	setThemeFinal();
})
CustomThemeSelected.addEventListener("click", function() {
	themeType = "custom"
	themeTypeSelect();
	setThemeFinal();
})
...
let theme = "dark";
let themeAccent = "orange";
let yu = themeAccent;
const input = document.querySelector('.colorselect');
input.addEventListener('input', Preview);
input.addEventListener('change', setCustom);

function setThemeFinal() {
	if (themeType === "preset") {
...
function setCustom(inputs) {
	root.style.setProperty(`--coloraccent`, inputs.target.value);
	if (localStorage.getItem("pomo-theme") === "dark") {
		root.style.setProperty(`--bgcolor`, "oklch(from var(--coloraccent) calc(l*0.3) calc(c*0.2) h)");
		root.style.setProperty(`--bgcolor2`, "oklch(from var(--coloraccent) calc(l*0.4) calc(c*0.8) h");
		root.style.setProperty(`--color`, "#ffffffff");
	} else if (localStorage.getItem("pomo-theme") === "light") {
		root.style.setProperty(`--bgcolor`, "oklch(from var(--coloraccent) calc(l*3) calc(c*0.3) h)");
		root.style.setProperty(`--bgcolor2`, "oklch(from var(--coloraccent) calc(l*1.7) calc(c*0.8) h)")
		root.style.setProperty(`--color`, "#222222");;
	}
	localStorage.setItem("custom-color", inputs.target.value);

	yu = localStorage.getItem("custom-color");
}
...
function LoadTheme(yu) {
	root.style.setProperty(`--coloraccent`, yu) ;
	if (localStorage.getItem("pomo-theme") === "dark") {
		root.style.setProperty(`--bgcolor`, "oklch(from var(--coloraccent) calc(l*0.3) calc(c*0.2) h)");
		root.style.setProperty(`--bgcolor2`, "oklch(from var(--coloraccent) calc(l*0.4) calc(c*0.8) h");
		root.style.setProperty(`--color`, "#ffffffff");
...
if (localStorage.getItem("custom-color")) {
	yu = localStorage.getItem("custom-color")
}
...
let socialslink = document.getElementById("fb");
let fblink = document.getElementById("link");
